...
app.get("/api/verify-email", async (req, res) => {
    ...
    try {
        ...
        await pool.request()
            .input("token", sql.NVarChar(255), token)
            .query(`UPDATE Users SET isVerified = 1, verificationToken = NULL, verificationTokenExpires = NULL WHERE verificationToken = @token`);
...
app.post("/api/forgot-password", async (req, res) => {
    ...
    try {
        ...
        await pool.request()
            ...
            .query(`UPDATE Users SET resetToken = @resetToken, resetTokenExpires = @resetTokenExpires WHERE email = @email`);
...
app.post("/api/reset-password", async (req, res) => {
    ...
    try {
        ...
        await pool.request()
            .input("token", sql.NVarChar(255), token)
            .input("password", sql.NVarChar(255), hashedPassword)
            .query(`UPDATE Users SET password = @password, resetToken = NULL, resetTokenExpires = NULL WHERE resetToken = @token`);
